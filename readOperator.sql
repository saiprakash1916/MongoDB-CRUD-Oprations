...
-- This one give to the based on matching values. If in case one match also it will gave the output
db.users.find({
    $and :[
        {"hobbies.title" : "Sports"},
        {"hobbies.frequency" :{
...
-- elemMatch is used for the to give the exact value.
db.users.find({
    hobbies :{
        $elemMatch :{
            title : "Sports",
...
--Finding the data  (1 means requried, 0 is not required )

db.movies.find({},{name:1, rating:1}).pretty()

db.movies.find({},{name:1, rating:1, _id:0}).pretty()

db.movies.find({},{name:1, rating:1, "rating.average" : 1 _id:0}).pretty()

-- This will return all values include other genres also
db.movies.find({genres:"Drama"}).pretty()

-- But this one only return Drama values
